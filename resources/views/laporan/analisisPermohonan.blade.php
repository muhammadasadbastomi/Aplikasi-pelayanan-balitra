...
<body>
    ...
    <div class="container">
        <div class="isi">
            <h2 style="text-align:center;">LAPORAN ANALISIS PERMOHONAN</h2>
            <table class="table table-bordered table-hover text-center">
                ...
                <tbody>
                    @foreach ($pelayanan as $r)
                        @foreach($r->pelayanan as $p)
                            <tr>
                                <td>{{$p->buah->name}}</td>
                                <td>{{$p->jenis_pelayanan->jenis}}</td>
                                <td>{{$p->jenis_pelayanan->permohonan->whereIn('status',[0,2])->count()}} Permohonan</td>
                            </tr>
                        @endforeach
                    @endforeach
                    </tfoot>
